...
const ConsultarReporte1 = () => {

    const navigation = useNavigation();
    const datos = useDatos()
    const header = ['Nombre', 'Número de Venta Cliente']
    console.log(datos)
    return (
        ...
        <ScrollView style={styles.scroll}>
          ...
          <Table borderStyle={{borderWidth: 1, borderColor: 'black', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
            ...
            {datos.map(item =>(
                <Row key={item.Nombre} data={[item.Nombre, item.NumberoVentasCliente]} style={{height: 90, backgroundColor: 'white'}} textStyle={{color: 'black',textAlign: 'center'}} /> 
            ))}
...
export default ConsultarReporte1
