...
const ConsultarReporte9 = () => {

    const navigation = useNavigation();
    const datos = useDatos()
    const header = ['Nombre','Valor Producto']
    console.log(datos)
    return (
        ...
        <ScrollView style={styles.scroll}>
          ...
          <Table borderStyle={{borderWidth: 1, borderColor: 'black', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
            ...
            {datos.map(item =>(
                <Row key={item.Nombre} data={[item.Nombre,item.ValorProducto]} style={{height: 90, backgroundColor: 'white'}} textStyle={{color: 'black',textAlign: 'center'}} /> 
            ))}
...
export default ConsultarReporte9
