...
function useDatos() {
    ...
    useEffect(() => {
      fetch(`http://${ip}/clientes/get-dataClientes`)
...
const ConsultarClientesCompleto = () => {
    ...
    return (
        ...
        <ScrollView style={styles.scroll}>
          ...
          <Table borderStyle={{borderWidth: 1, borderColor: 'black', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
            ...
            {datos.map(item =>(
                <Row key={item.id_Cliente} data={[item.id_Cliente, item.id_Direccion,item.Nombre, item.Correo, item.Estatus]} style={{height: 90, backgroundColor: 'white'}} textStyle={{color: 'black',textAlign: 'center'}} /> 
            ))}
...
export default ConsultarClientesCompleto
