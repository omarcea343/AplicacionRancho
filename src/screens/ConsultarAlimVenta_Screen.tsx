...
function useDatos() {
    ...
    useEffect(() => {
      fetch(`http://${ip}/alimentoVenta/get-dataAlimentoVenta`)
...
const ConsultarAlimVentaCompleto = () => {
    ...
    return (
        ...
        <ScrollView style={styles.scroll}>
          ...
          <Table borderStyle={{borderWidth: 1, borderColor: 'black', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
            ...
            {datos.map(item =>(
                <Row key={item.id} data={[item.id, item.Nombre,item.PrecioUnitario, item.Cantidad, item.TipoUnidad]} style={{height: 90, backgroundColor: 'white'}} textStyle={{color: 'black',textAlign: 'center'}} /> 
            ))}
...
export default ConsultarAlimVentaCompleto
