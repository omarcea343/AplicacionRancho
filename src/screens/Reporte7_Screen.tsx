...
const ConsultarReporte7 = () => {

    const navigation = useNavigation();
    const datos = useDatos()
    const header = ['ID Alimento','Tipo de Animal', 'Comida Sugerida']
    console.log(datos)
    return (
        ...
        <ScrollView style={styles.scroll}>
          ...
          <Table borderStyle={{borderWidth: 1, borderColor: 'black', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
            ...
            {datos.map(item =>(
                <Row key={item.id_alim} data={[item.id_alim,item.Tipo_animal,item.Comida_Sugerida]} style={{height: 90, backgroundColor: 'white'}} textStyle={{color: 'black',textAlign: 'center'}} /> 
            ))}
...
export default ConsultarReporte7
